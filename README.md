...
- 작업 상세 DB는 선택적으로 `NOTION_WORK_ITEMS_DATA_SOURCE_ID`를 추가 설정할 수 있습니다.

현재 등록된 개인 프로젝트:

- `C:\Users\ISDATA\Desktop\test2`
- `C:\Users\ISDATA\Desktop\test`
- `C:\Users\ISDATA\Desktop\auto_blog`
...
- `save log` 요청 시 등록된 프로젝트들의 오늘 Git 커밋을 먼저 수집해 `worklog.json` 초안을 갱신한 뒤 오늘 날짜 페이지를 생성하거나 업데이트합니다.
- 날짜는 항상 오늘 날짜를 사용합니다.
...
- Git 저장소인 프로젝트만 대상으로 오늘 커밋을 읽습니다.
...
작업 상세를 별도 DB에 저장하려면 두 번째 데이터베이스를 아래 속성으로 추가할 수 있습니다.

- `Task` : Title
- `Work Date` : Date
- `Duration` : Number
- `Category` : Rich text 또는 Select/Status
- `Project` : Rich text 또는 Select/Status
- `Tool` : Rich text 또는 Select/Status
- `Start Time` : Rich text 또는 Select/Status
- `End Time` : Rich text 또는 Select/Status

상세 DB 속성명은 아래 8개를 공백까지 정확히 맞춰야 합니다.
...
작업 상세 DB의 위 속성들은 코드가 실제 Notion 속성 타입을 조회해서
`rich_text`, `select`, `status`에 맞게 자동 전송합니다.
```
...
NOTION_WORK_ITEMS_DATA_SOURCE_ID=yyyyyyyyyyyyyyyy
```
...
- `NOTION_WORK_ITEMS_DATA_SOURCE_ID`가 설정돼 있으면 같은 날짜 작업 상세 row도 별도 DB에 다시 저장합니다.
...
```text
[GMLIM_260317] Update : 노션 데이터 베이스저장시 기본템플릿으로 변경
```
